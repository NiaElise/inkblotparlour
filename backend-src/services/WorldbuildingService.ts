...
import { Character, Lore, Secret, TensionMap, User } from '../types/index';
import { UserService } from './UserService';

export class WorldbuildingService {
  // Characters and Lore are open to all tiers (with limits mentioned in business plan, but let's focus on gating first)
  ...
  // Secrets and Tension Maps are feature-gated to Paid tiers (Architect, Collective)
  
  static createSecret(userId: string, storyworldId: string, id: string, title: string, content: string): void {
    const user = UserService.getUser(userId);
    if (!user) throw new Error('User not found');

    if (user.tier === 'Draftsman') {
      throw new Error('Secrets (Secret Web) are only available for Architect and Collective tiers.');
    }
  ...
  static getSecrets(userId: string, storyworldId: string): Secret[] {
    const user = UserService.getUser(userId);
    if (!user) throw new Error('User not found');
    if (user.tier === 'Draftsman') throw new Error('Secret Web is only available for Architect and Collective tiers.');
  ...
  static getTensionMaps(userId: string, storyworldId: string): TensionMap[] {
    const user = UserService.getUser(userId);
    if (!user) throw new Error('User not found');
    if (user.tier === 'Draftsman') throw new Error('Tension Mapping is only available for Architect and Collective tiers.');
  ...
  static getTimelines(userId: string, storyworldId: string): any[] {
    const user = UserService.getUser(userId);
    if (!user) throw new Error('User not found');
    if (user.tier === 'Draftsman') throw new Error('Timeline Orchestration is only available for Architect and Collective tiers.');
  ...
  static getSecretSocieties(userId: string, storyworldId: string): any[] {
    const user = UserService.getUser(userId);
    if (!user) throw new Error('User not found');
    if (user.tier === 'Draftsman') throw new Error('Secret Societies are only available for Architect and Collective tiers.');
